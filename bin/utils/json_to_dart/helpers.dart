...
const Map<String, bool> premitiveTypes = {
  'int': true,
  'double': true,
  'String': true,
  'bool': true,
...
enum ListType { typeObject, typeString, typeDouble, typeInt, typeNull }
...
MergeableListType mergeableListType(List<dynamic> list) {
  ListType t = ListType.typeNull;
  bool isAmbigous = false;
  for (var e in list) {
    ListType? inferredType = getInferredType(e);
    if (t != ListType.typeNull && t != inferredType) {
      isAmbigous = true;
    }
    t = inferredType ?? ListType.typeNull;
  }
  ...
  other.forEach((k, v) {
    if (clone[k] == null) {
      ...
      if (t != otherType) {
        ...
        if (ListType.typeObject == mergeableType.listType) {
...
isPrimitiveType(String typeName) {
  final isPrimitive = premitiveTypes[typeName];
  if (isPrimitive == null) {
    return false;
  }
  return isPrimitive;
