...
class TypeDefinition {
  ...
  @override
  int get hashCode => hash2(name.hashCode, subtype?.hashCode);
  ...
  String jsonParseExpression(String key, bool privateField) {
    ...
    if (isPrimitive) {
      if (name == "List") {
        return "var $fieldKey = json['$key'].cast<$subtype>();";
      }
      return "var $fieldKey = json['$key'];";
    } else if (name == "List" && subtype == "DateTime") {
      return "var $fieldKey = json['$key'].map((v) => DateTime.tryParse(v));";
    } else if (name == "DateTime") {
      return "var $fieldKey = DateTime.tryParse(json['$key']);";
    } else if (name == 'List') {
      // list of class
      return "List<$subtype>? $fieldKey;\nif (json['$key'] != null) {\n\t\t\t$fieldKey = [];\n\t\t\tjson['$key'].forEach((v) { $fieldKey!.add($subtype.fromJson(v)); });\n\t\t}";
    } else {
      // class
      return "var $fieldKey = json['$key'] != null ? ${_buildParseClass(jsonKey)} : null;";
    }
  }
...
class ClassDefinition {
  ...
  @override
  int get hashCode => hash2(name.hashCode, _privateFields.hashCode);
